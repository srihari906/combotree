...
;(function ( $, window, document, undefined ) {
    ...
    ComboTree.prototype.singleItemClick = function (ctItem) {
        ...
        if (!this.options.isMultiple) {
            for(var k=0; k<this._elemItems.length; k++){
                var dataid = $(this._elemItems[k]).children('span.comboTreeItemTitle').attr('data-id');
                for (var i=0; i<this._selectedItems.length; i++){
                    if (parseInt(dataid) == parseInt(this._selectedItems[i].id)){
                        if(parseInt(dataid) == parseInt($(ctItem).attr('data-id'))){
                            $(this._elemItems[k]).children('span.comboTreeItemTitle').children('input').prop("checked", true);
                        }else{
                            for(var j=0; j<this._selectedItems.length; j++){
                                if(this._selectedItems[j].id == parseInt(dataid))
                                    this._selectedItems.splice(this._selectedItems[j], 1)
                            }
                            $(this._elemItems[k]).children('span.comboTreeItemTitle').children('input').prop("checked", false);
                        }
                    }
                }
            }
